...
function PostCard({ post, categoryColor, formatTimeAgo }) {
  const [isLiked, setIsLiked] = useState(false);
  const [likeCount, setLikeCount] = useState(post.likeCount);
  const isFeatured = post.title === '육아 번아웃 왔어요...';
  const avatarSrc = isFeatured ? (post.authorImage || photoMom) : todocCharacter;
      ...
      <article className="post-card">
        <div className="post-card-header">
          <img
            src={avatarSrc}
            alt="프로필"
            className={`post-card-avatar ${isFeatured ? '' : 'post-card-avatar-gray'}`}
          />
        ...
        <div className="post-card-stats">
          <button
            type="button"
            className={`post-card-stat post-card-like ${isLiked ? 'liked' : ''}`}
            onClick={() => {
